"""Claude Code 用户级 Rules 和 Skills 目标."""

from __future__ import annotations

from pathlib import Path

from ..common import InstallResult, InstallStatus, RuleSource, SkillSource, copy_tree


def rules_root(user_home: Path, variant: str | None = None) -> Path:
    del variant
    return user_home / ".claude" / "rules"
...
def install_rules(
    ...
    results = []
    for rule in rules:
        target = destination / rule.relative_path
...
def list_status(
    ...
    return [
        *[
            InstallStatus("rule", str(rule.relative_path), rules_path / rule.relative_path,
                          (rules_path / rule.relative_path).is_file())
            for rule in rules
        ],
        *[
